...
object TestUtils {
  ...
  case object UShort extends DataType

  implicit class RichBufferedImage(val self: BufferedImage) extends AnyVal {
    ...
    def compareEachPixel(that: BufferedImage)(withFilter: Int => Boolean)(pred: (Int, Int) => Boolean) = {
      ...
      @tailrec def rec(idx: Int = 0, ret: Boolean = true): Boolean = if (idx < selfPix.length) ret else {
